...
def get_data(file_dir: List[str], data_type: str):
    ...
    ## Return.
    return _get_book_data(file_dir) if data_type == "book" else _get_paper_data(file_dir)
...
class BARTAbstractiveSummarizationDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        ## Load raw data.
        self.raw_data = list(itertools.chain.from_iterable([
            get_data(file_dir, data_type=Path(file_dir).parts[-2]) for file_dir in file_dirs
        ]))
        ...
        } for instance in tqdm.tqdm(self.raw_data, desc="Cleaning", total=len(self.raw_data))]
        LOGGER.info(f"[{self.mode}] All data cleaned: {', '.join(file_dirs)}")

        ## todo: tokenize using multi-processing.
        self.data = [{
            key: np.array(self.tokenizer.encode(value)) for key, value in instance.items()
        } for instance in tqdm.tqdm(self.data, desc="Tokenizeing", total=len(self.data))]
    ...
    def __getitem__(self, idx):
        ...
        ## Inputs for encoder.
        input_ids = self._pad(instance["input"], token_id=self.tokenizer.pad_token_id) ## w/o special tokens
        attention_mask = (input_ids != self.tokenizer.pad_token_id).astype(float)

        ## Inputs for decoder (generator).
        decoder_input_ids = np.concatenate([[self.tokenizer.eos_token_id], instance["label"]])
        decoder_input_ids = self._pad(decoder_input_ids, token_id=self.tokenizer.pad_token_id)
        decoder_attention_mask = (decoder_input_ids != self.tokenizer.pad_token_id).astype(float)
        ...
        return {
            ...
            "decoder_attention_mask":   torch.from_numpy(decoder_attention_mask).to(torch.long),
